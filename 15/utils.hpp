...
template <typename T, typename F> void order(T begin, T end, F const &f) {
...
std::string join(std::vector<std::string> const &v, std::string c = "") {
  std::ostringstream oss;
  if (v.size() == 0)
    return "";
  oss << v[0];
  for (const auto &s : v | std::views::drop(1))
    oss << c << s;
  return oss.str();
}

template <typename T> std::string padded(T const &x, size_t n, char c = ' ') {
  std::ostringstream oss;
...
std::string cleaned(std::string s) {
